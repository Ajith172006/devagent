...
export function renderPortfolioHtml(
  data: PortfolioData,
  options: { githubUsername?: string; displayName?: string; includeLeetcode?: boolean } = {},
): string {
  ...
  const githubLink =
    ...
    (options.githubUsername ? `https://github.com/${options.githubUsername}` : '');
  ...
  const allProjectTags = Array.from(new Set(projects.flatMap((p: any) => p.tech || []))).slice(
    0,
    12,
  ) as string[];
  ...
  const themeIndex = getThemeIndex(data.user?.id || 'dev', 4);
    ...
    .map(
      ...
      <div class="project-card reveal" data-tech="${(proj.tech || []).map(escapeHtml).join(',')}">
        ...
        <div class="project-tech">
          ${(proj.tech || []).map((t: string) => `<span class="tag">${escapeHtml(t)}</span>`).join('')}
        </div>
      </div>`,
    )
    .join('');
